...
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=244104462784.apps.googleusercontent.com&access_type=offline&scope=https://www.googleapis.com/auth/analytics.readonly&redirect_uri=HTTP://localhost/
ko added url to grant access to analytics
code=4/jv3w2bMSIDoQG9EMa6A-NqlJS_Yj ko added token from browser after hitting above url
...
def main(argv):
  ga_sample_utils.process_flags(argv)

  # Authenticate and construct service.
  service = ga_sample_utils.initialize_service()

  # Try to make a request to the API. Print the results or handle errors.
  try:
    first_profile_id = "44826445"
...
def get_top_keywords(service, profile_id):
  ...
  return service.data().ga().get(
      ...
      sort='-ga:visits',
      filters='ga:medium==organic;ga:keyword==nsac',
      start_index='1',
      max_results='35').execute()
