...
def date_select(get_request, client_id):
    ...
    first_data_date = LogSeRank.objects \
                               .values('refdate') \
    ...
    last_data_date = LogSeRank.objects \
                              .values('refdate') \
                              .filter(client_id=client_id) \
                              .order_by('-refdate')[0]
...
def last_full_week(client_id):
    ...
    latest_sunday  = LogSeRank.objects \
                              .values('refdate') \
...
def process_time_series(query,
    ...
    # e.g., days,weeks (default is days)
    time_series = qss.time_series(start_date, end_date, agg_interval)
